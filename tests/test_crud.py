...
def test_update_bagger(db):
    ...
    # Create a bagger
    original_data = schemas.BaggerCreate(
        name="Original Name",
        membershipNo="AFL555"
    )
    ...
    updated_bagger = crud.update_bagger(db=db, bagger_id=created_bagger.id, bagger=update_data)
...
def test_unique_membership_constraint(db):
    ...
    bagger1 = schemas.BaggerCreate(
        name="First User",
        membershipNo="AFL123"
    )
    bagger2 = schemas.BaggerCreate(
        name="Second User",
        membershipNo="AFL123"  # Same membership number
    )
